...
static void nf_ct_offload_timeout(struct flow_offload *flow)
{
	struct flow_offload_entry *entry;
	struct nf_conn *ct;
	unsigned long newtime = jiffies + DAY / 2;
...
void flow_offload_teardown_by_tuple(struct flow_offload_tuple *tuple)
{
	...
	/*Fix me: how get the correct flow table pointer*/
			  ...
			  void *data)
{
	...
	err = rhashtable_walk_init(&flow_table->rhashtable, &hti);
	if (err)
		return err;
...
void nf_flow_counter_refresh(struct flow_offload *flow,
				enum flow_offload_tuple_dir dir,
				struct sk_buff *skb)
{
	struct flow_offload_entry *entry;
...
EXPORT_SYMBOL_GPL(nf_flow_counter_refresh);
...
static struct notifier_block flow_table_netdev_notifier = {
	.notifier_call	= nf_flow_table_netdev_event,
};

static int __init nf_flow_table_module_init(void)
{
	return register_netdevice_notifier(&flow_table_netdev_notifier);
}

static void __exit nf_flow_table_module_exit(void)
{
	unregister_netdevice_notifier(&flow_table_netdev_notifier);
}
