		...
		struct dma_slave_config *config)
{
	...
	if (config->device_fc) {
		dev_notice(dma_dev->ddev.dev, "not support flow controller\n");
		return -EINVAL;
	}
	...
	case DMA_MEM_TO_DEV:
		if (config->dst_addr_width != DMA_SLAVE_BUSWIDTH_4_BYTES) {
			dev_notice(dma_dev->ddev.dev, "only support 4 byte buswidth\n");
			return -EINVAL;
		}
	...
	case DMA_DEV_TO_MEM:
		...
		break;
	default:
		dev_notice(dma_dev->ddev.dev, "direction type %d error\n",
				config->direction);
		return -EINVAL;
	}
...
static int gdma_dma_terminate_all(struct dma_chan *c)
{
	...
	while (gdma_dma_read(dma_dev, GDMA_REG_CTRL0(chan->id)) &
		...
		if (time_after_eq(jiffies, timeout)) {
			dev_notice(dma_dev->ddev.dev, "chan %d wait timeout\n",
					chan->id);
...
static void rt305x_dump_reg(struct gdma_dma_dev *dma_dev, int id)
{
	...
	dev_dbg(dma_dev->ddev.dev, "\tctr1 %08x, intr %08x, signal %08x\n",
...
static int rt305x_gdma_start_transfer(struct gdma_dmaengine_chan *chan)
{
	...
	if (unlikely(ctrl0 & GDMA_REG_CTRL0_ENABLE)) {
		dev_notice(dma_dev->ddev.dev, "chan %d is start(%08x).\n",
				chan->id, ctrl0);
...
static void rt3883_dump_reg(struct gdma_dma_dev *dma_dev, int id)
{
	...
	dev_dbg(dma_dev->ddev.dev, "\tctr1 %08x, unmask %08x, done %08x\n",
			gdma_dma_read(dma_dev, GDMA_REG_CTRL1(id)),
			gdma_dma_read(dma_dev, GDMA_REG_UNMASK_INT),
			gdma_dma_read(dma_dev, GDMA_REG_DONE_INT));

	dev_dbg(dma_dev->ddev.dev, "\treq %08x, ack %08x, fin %08x\n",
		...
		unsigned long flags, void *context)
{
	...
	if (!desc) {
		dev_notice(c->device->dev, "alloc sg decs error\n");
		return NULL;
	}
	desc->residue = 0;
	...
	for_each_sg(sgl, sg, sg_len, i) {
		if (direction == DMA_MEM_TO_DEV)
			...
			dev_notice(c->device->dev, "direction type %d error\n",
					direction);
			goto free_desc;
		}

		if (unlikely(sg_dma_len(sg) > GDMA_REG_CTRL0_TX_MASK)) {
			dev_notice(c->device->dev, "sg len too large %d\n",
					sg_dma_len(sg));
			goto free_desc;
		}
		...
		size_t len, unsigned long flags)
{
	...
	if (!desc) {
		dev_notice(c->device->dev, "alloc memcpy decs error\n");
		return NULL;
	}
	desc->residue = len;
	...
	unsigned long flags)
{
	...
	if (period_len > GDMA_REG_CTRL0_TX_MASK) {
		dev_notice(c->device->dev, "cyclic len too large %d\n",
				period_len);
		return NULL;
	}
	...
	desc = gdma_dma_alloc_desc(num_periods);
	if (!desc) {
		dev_notice(c->device->dev, "alloc cyclic decs error\n");
		return NULL;
	}
...
static const struct of_device_id gdma_of_match_table[] = {
	{ .compatible = "mtk,rt305x-gdma", .data = &rt305x_gdma_data },
	{ .compatible = "mtk,rt3883-gdma", .data = &rt3883_gdma_data },
	{ },
};
...
static int gdma_dma_probe(struct platform_device *pdev)
{
	...
	dma_dev = devm_kzalloc(&pdev->dev, sizeof(*dma_dev) +
			...
			GFP_KERNEL);
	if (!dma_dev) {
		dev_notice(&pdev->dev, "alloc dma device failed\n");
		return -EINVAL;
	}
	dma_dev->data = data;
	...
	if (irq < 0) {
		dev_notice(&pdev->dev, "failed to get irq\n");
		return -EINVAL;
	}
	ret = devm_request_irq(&pdev->dev, irq, gdma_dma_irq,
			0, dev_name(&pdev->dev), dma_dev);
	if (ret) {
		dev_notice(&pdev->dev, "failed to request irq\n");
		return ret;
	}
	...
	if (ret) {
		dev_notice(&pdev->dev, "failed to register dma device\n");
		return ret;
	}
	...
	if (ret) {
		dev_notice(&pdev->dev, "failed to register of dma controller\n");
		goto err_unregister;
	}
...
MODULE_DESCRIPTION("MediaTek DMA driver");
