...
static void hsdma_dump_reg(struct mtk_hsdam_engine *hsdma)
{
	...
	dev_dbg(hsdma->ddev.dev, "tctx %08x, tdtx %08x, rbase %08x\n",
			mtk_hsdma_read(hsdma, HSDMA_REG_TX_CTX),
			mtk_hsdma_read(hsdma, HSDMA_REG_TX_DTX),
			mtk_hsdma_read(hsdma, HSDMA_REG_RX_BASE));

	dev_dbg(hsdma->ddev.dev, "rcnt %08x, rctx %08x, rdtx %08x\n",
	...
	dev_dbg(hsdma->ddev.dev, "intr_stat %08x, intr_mask %08x\n",
		...
		struct mtk_hsdma_chan *chan)
{
	...
	for (i = 0; i < HSDMA_DESCS_NUM; i++) {
		...
		dev_dbg(hsdma->ddev.dev, "tx addr1: %08x, rx addr0 %08x, flags %08x\n",
...
static int mtk_hsdma_terminate_all(struct dma_chan *c)
{
	...
	while (mtk_hsdma_read(hsdma, HSDMA_REG_GLO_CFG) &
		...
		if (time_after_eq(jiffies, timeout)) {
			...
			dev_notice(hsdma->ddev.dev, "timeout, reset it\n");
			break;
		}
		cpu_relax();
	}
...
static int mtk_hsdma_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_notice(&pdev->dev, "failed to alloc ring descs\n");
		return ret;
	}
