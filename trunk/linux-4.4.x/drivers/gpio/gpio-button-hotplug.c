...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		gpio_direction_input(button->gpio); //Setting all the button state is input mode. In MTK SDK, the default reset button state is output mode, we need to do this.
	}
