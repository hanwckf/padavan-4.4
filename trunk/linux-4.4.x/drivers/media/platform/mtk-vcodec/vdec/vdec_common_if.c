...
static void put_fb_to_free(struct vdec_inst *inst, struct vdec_fb *fb)
{
	...
	if (fb) {
		...
		mtk_vcodec_debug(inst, "[FB] put fb into free_list @(%p, %llx)",
				 fb->planes[0].va, (u64)fb->planes[0].dma_addr);
			       ...
			       struct vdec_fb *fb, bool *res_chg)
{
	...
	y_fb_dma = fb ? (u64)fb->planes[0].dma_addr : 0;
	c_fb_dma = fb ? (u64)fb->planes[1].dma_addr : 0;
	...
	if (fb) {
		inst->vsi->dec.y_fb_fd = get_mapped_fd(fb->planes[0].dmabuf);
		inst->vsi->dec.c_fb_fd = get_mapped_fd(fb->planes[1].dmabuf);
	} else {
