...
static const struct mtk_gate ssusb_clks[] __initconst = {
	GATE_SSUSB(CLK_SSUSB_U2_PHY_1P_EN, "ssusb_u2_phy_1p", "to_u2_phy_1p",
		0),
	GATE_SSUSB(CLK_SSUSB_U2_PHY_EN, "ssusb_u2_phy_en", "to_u2_phy",
		1),
	GATE_SSUSB(CLK_SSUSB_REF_EN, "ssusb_ref_en", "to_usb3_ref",
		5),
	GATE_SSUSB(CLK_SSUSB_SYS_EN, "ssusb_sys_en", "to_usb3_sys",
		6),
	GATE_SSUSB(CLK_SSUSB_MCU_EN, "ssusb_mcu_en", "to_usb3_mcu",
		7),
	GATE_SSUSB(CLK_SSUSB_DMA_EN, "ssusb_dma_en", "to_usb3_dma",
		8),
};
...
CLK_OF_DECLARE(mtk_ssusbsys, "mediatek,leopard-ssusbsys", mtk_ssusbsys_init);
...
static void __init mtk_pciesys_init(struct device_node *node)
{
	...
	if (of_device_is_compatible(node, "mediatek,leopard-pciephy"))
		mtk_register_reset_controller(node, 1, 0x00);
	else {
...
CLK_OF_DECLARE(mtk_pciesys, "mediatek,leopard-pciesys", mtk_pciesys_init);
CLK_OF_DECLARE(mtk_pciephy, "mediatek,leopard-pciephy", mtk_pciesys_init);
