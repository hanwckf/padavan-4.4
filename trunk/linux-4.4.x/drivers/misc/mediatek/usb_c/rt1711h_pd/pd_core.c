...
static int pd_parse_pdata(struct pd_port_t *pd_port)
{
	...
	struct pd_port_t *pd_port, struct svdm_svid_data_t *svid_data)
{
	...
	WARN_ON(pd_port->dp_second_connected == DPSTS_BOTH_CONNECTED);
	/* UFP or DFP can't both be invalid */
	WARN_ON(ufp_d_pin_cap == 0 && dfp_d_pin_cap == 0);
	if (pd_port->dp_first_connected == DPSTS_BOTH_CONNECTED) {
		WARN_ON(ufp_d_pin_cap == 0);
		WARN_ON(dfp_d_pin_cap == 0);
	}
...
static void pd_core_parse_svid_data(struct pd_port_t *pd_port)
{
	int ret, i = 0;
...
static void pd_core_power_flags_init(struct pd_port_t *pd_port)
{
...
static inline int pd_reset_modal_operation(struct pd_port_t *pd_port)
{
	uint8_t i;
	struct svdm_svid_data_t *svid_data;
...
static int pd_send_message(struct pd_port_t *pd_port, uint8_t sop_type,
...
int pd_send_ctrl_msg(struct pd_port_t *pd_port, uint8_t sop_type, uint8_t msg)
{
...
int pd_reply_svdm_request(struct pd_port_t *pd_port, struct pd_event_t *pd_event,
